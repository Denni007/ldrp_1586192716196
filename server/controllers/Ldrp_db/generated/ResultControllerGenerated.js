...
const generatedControllers = {
  ...
  init: router => {
    ...
    router.get(baseUrl + "/findBystudent/:key", authorize([]), ResultController.findBystudent);
  ...
  */
  findBystudent: async (req, res) => {
    try {
      const result = await ResultModel.findBystudent(req.params.key);
      res.json(result);
    } catch (err) {
      const safeErr = ErrorManager.getSafeError(err);
      res.status(safeErr.status).json(safeErr);
